...
namespace MultyThreadTCPListener
{
    public partial class Scheduler : Form
    {
        ...
        {
            if (MessageBox.Show("Do yuo really want to delete ALL the messages?", "Delete all Queried messages", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                RemoveAll();
                    ...
                    if (listView1.Items[i].Checked == true)
                    {
                        for (int ind = 1; ind <= str1.Length - 1; ind++)
                        {
                            if (str1[ind] == "")
                            {
                                ...
                                string comm2 = "UPDATE [MessageSchedule] SET message" + ind.ToString() + "= " + "'" + this.textBox_ScheduleMessage.Text.ToString() + "'" + "WHERE FKUnit_ID="+str1[0];
